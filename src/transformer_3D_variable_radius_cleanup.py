...
file_path = "../data/CA3D/CA3D_random_radius_10k.xlsx"
...
def plot_sample_interactive_from_input(model, test_input):
    ...
    radius = test_input[0, 3 + numObs, 0]
...
def plot_dataset_sample(model, ds, idx, save_path=None, title_prefix="test",
    ...
    radius = X_denorm[3+numObs, 0].item()
...
@torch.no_grad()
def count_collisions(model, loader):
    model.eval()
    device = next(model.parameters()).device
    ...
    for X, _ in loader:
        ...
        # Extract radius for each sample and denorm
        radius_norm = X[:, 3 + numObs, :]  # Token after control point
        radius_den = radius_norm * X_std_t[:, 3 + numObs, :] + X_mean_t[:, 3 + numObs, :]
        radius_batch = radius_den[:, 0]  # (B,) - first component
        ...
        for obs_idx in range(numObs):
            ...
            inside = (dist < radius_batch[:, None])  # Broadcast (B,) against (B, T_out)
...
@torch.no_grad()
def count_collisions_continuous(model, loader, buffer=0.0, n_eval=100):
    model.eval()
    device = next(model.parameters()).device
    ...
    for X, _ in loader:
        ...
        for i in range(X.size(0)):
            ...
            # Extract radius for this sample
            radius = X_den[i, 3 + numObs, 0].cpu().item()  # Scalar
            ...
            # Check collision with ANY obstacle using dynamic radius
            has_collision = False
...
# Stack into tokens
X_np = np.stack([start, end, *obstacles, control, in_radius], axis=1)
