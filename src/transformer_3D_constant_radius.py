...
# ======================================================================================================================
def plot_nasa_prediction(test_input_raw, pred_points, save_path=None, title=None, show=False):
    # Unpack raw tokens
    ...
    fig.update_layout(
        ...
        title=title or "NASA Prediction"
    )
...
if NASA_EVAL:
    ...
    # Load NASA data
    NASA_file_path = "../data/BAM/normalized_extracted_data.xlsx"
    df_nasa = pd.read_excel(NASA_file_path)
    ...
    # Output directory
    output_dir = "figs/nasa_predictions"
    ...
    for i in range(num_rows):
        ...
        test_norm = (test_raw - X_mean) / X_std
        test_tensor = torch.from_numpy(test_norm).float().to(device)

        # Predict (normalized output)
        with torch.no_grad():
            out_norm = model(test_tensor)

        # Denorm predictions
        out = out_norm.cpu().numpy() * Y_std + Y_mean
        pred_points = out[0]
        ...
        for j in range(7):
            row[f"cp{j + 2}_x"] = pred_points[j, 0]
            row[f"cp{j + 2}_y"] = pred_points[j, 1]
            row[f"cp{j + 2}_z"] = pred_points[j, 2]

        rows.append(row)

    # Convert to df and save
    df_out = pd.DataFrame(rows)
    xlsx_out = f"{output_dir}/nasa_predictions.xlsx"
    df_out.to_excel(xlsx_out, index=False)

    print(f"\nSaved all predictions to {xlsx_out}")
