...
def plot_manual_trajectory(pred_3xT, manual_input: dict, run_dir: Path, args):
    """Plot trajectory from manual input using BeBOT"""
    ...
    vx, vy, vz = manual_input['vx'], manual_input['vy'], manual_input['vz']
    ...
    ax.plot(traj_x, traj_y, traj_z, 'b-', linewidth=2, label='trajectory', alpha=0.8)

    # Plot control points
    ax.scatter(pred_points[:, 0], pred_points[:, 1], pred_points[:, 2],
               s=28, marker="o", c='green', label="control points", depthshade=False)
...
def main():
    ...
    if args.manual:
        ...
        if args.animate_radius:
            animate_radius_sweep(model, test_input, save_path="tools/extra/radius_sweep_fnn.gif", elev=45, azim=-70,
                                 n_eval=50)
            return
    ...
    for j, idx in enumerate(indices[:args.num_samples]):
        ...
        ox1, oy1, oz1 = x_row[6].item(), x_row[7].item(), x_row[8].item()
        ox2, oy2, oz2 = x_row[9].item(), x_row[10].item(), x_row[11].item()
        ox3, oy3, oz3 = x_row[12].item(), x_row[13].item(), x_row[14].item()
        r = 0.05
        ...
        if args.show_gt:
            ...
            if args.show_obs:
                ...
                for obs_idx, (ox, oy, oz) in enumerate([(ox1, oy1, oz1), (ox2, oy2, oz2), (ox3, oy3, oz3)]):
                    xs = ox + r * np.cos(u) * np.sin(v)
                    ys = oy + r * np.sin(u) * np.sin(v)
                    zs = oz + r * np.cos(v)
